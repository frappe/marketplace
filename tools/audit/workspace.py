...
from get_app_check import frappe_branch_for
...
def python_version_for(checkout: Path) -> str:
    ...
    3.14 - so the environment has to follow the branch, not the machine.
    """
    ...
    try:
        allowed = SpecifierSet(requires) if requires else None
...
class ReleaseWorkspace:
    ...
    def __enter__(self) -> tuple[App, Workspace]:
        ...
        branch = frappe_branch_for(self.release.get("frappe_core", ""))
